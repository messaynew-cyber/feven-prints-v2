...
### TRUST SET — privacy / contact / about ✅ **SHIPPED 2026-09-23** (`b591941`, fix `f022616`)
`/privacy` · `/contact` · `/about` — generated by `build-pages.js`, in the footer and the sitemap (**11 URLs**).
**Why now:** the site takes names, phone numbers and photographs. A shop that asks for a customer's pictures and
says nothing about what happens to them has not earned the pictures.
- `/privacy` in plain language, both languages: what is collected, that files sit in a private R2 bucket, that
  they are deleted automatically after 30 days, that nothing is sold or reused, what WhatsApp does with the
  conversation, what lives only on the customer's device.
- 🔴 **The wording was corrected to match the mechanism:** the uploader said "deleted 30 days after you collect";
  the R2 lifecycle rule deletes **30 days after they are sent**. The copy now says what actually happens.
- `/about` reuses the reprint promise **already in the FAQ** rather than inventing a policy Feven has not decided.
- ⚠️ **New Amharic on all three pages needs Feven's proofread.**
- **Two bugs of mine, both caught by eyes rather than by tests:** the renderer read each paragraph pair as a list
  and published **one letter per paragraph** ("T") — the build passed, every test passed, and a screenshot caught
  it. Then my first attempt to wire in the guard **did nothing at all** (a string replace that did not match,
